...
module.exports = class Address {
	...
	static async findById(id) {
		const results = await executeQuery(`SELECT * FROM addresses WHERE id = ?`, [id,]);
	...
	static async deleteById(id) {
		const result = await executeQuery(`DELETE FROM addresses WHERE id = ?;`, [id,]);
    ...
    async update() {
		const result = await executeQuery(
			`UPDATE addresses SET country = ?, county = ?, municipality = ?, zipCode = ?, city = ?, street = ?, street_number = ?, apartment_number = ? WHERE id = ?`,
			[
				this.country,
				this.county,
				this.municipality,
